...
SELECT 
    ...
    p.Likes,
    p.Dislikes,
    CAST(p.Likes AS FLOAT) / NULLIF(p.Dislikes, 0) AS Like_Dislike_Ratio
FROM Video_Info v
...
CREATE TABLE Audience (
    ...
    Audience_Retention_Rate DECIMAL(4,1),
    Click_Through_Rate DECIMAL(4,1)
);
...
SELECT 
    ...
    p.Likes,
    p.Dislikes,
    CAST(p.Likes AS FLOAT) / NULLIF(p.Dislikes, 0) AS LTD_Ratio
FROM Video_Info v
JOIN Performance_Metrics p
    ON v.Video_ID = p.Video_ID;
...
SELECT 
    ...
    CAST(p.Views AS FLOAT) / NULLIF(p.Watch_Time_Minutes, 0) AS VTW_Ratio
FROM Video_Info v
JOIN Performance_Metrics p
    ON v.Video_ID = p.Video_ID
ORDER BY VTW_Ratio ASC;
